...
class PACE {
  ...
  static Uint8List generateEncodingInputData(
    ...
    try {
      ...
      if (ephemeralPublic.agreementAlgorithm == TOKEN_AGREEMENT_ALGO.ECDH) {
        // ECDH
        const pointMarker = 0x04;
        Uint8List uncompressedPointData = Uint8List.fromList([pointMarker]);
        publicKeyData = TLV(
            ellipticCurvePoint,
            Uint8List.fromList(
                uncompressedPointData + ephemeralPublic.toBytes()));
  ...
  static Uint8List generateAuthenticationTemplateForMutualAuthenticationData(
    ...
    TLV cm = TLV(cyryptographicMechanismRefTag,
  ...
  static Uint8List generateGeneralAuthenticateDataStep2and3(
    ...
    if (public.agreementAlgorithm == TOKEN_AGREEMENT_ALGO.ECDH) {
      ...
      const pointMarker = 0x04;
      Uint8List uncompressedPointData = Uint8List.fromList([pointMarker]);
      mappingData = TLV(publicKeyTag,
          Uint8List.fromList(uncompressedPointData + public.toBytes()));
    ...
    TLV dynamicAuthenticationData =
        TLV(dynamicAuthenticationDataTag, mappingData.toBytes());
  ...
  static Uint8List generateGeneralAuthenticateDataStep4(
    ...
    TLV dynamicAuthenticationData =
        TLV(dynamicAuthenticationDataTag, authenticationToken.toBytes());
    ...
    try {
      ...
      if (cipherAlgo == CipherAlgorithm.AES) {
        ...
        Uint8List decryptedNonce =
            DESedeDecrypt(edata: nonce, key: kPi, iv: Uint8List(8));
        _log.sdVerbose(
            "PACE.decryptNonce; Decrypted nonce: ${decryptedNonce.hex()}");
        return decryptedNonce;
      } else {
